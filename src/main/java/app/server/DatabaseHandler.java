...
public class DatabaseHandler implements ConnectionHandlerInterface{
    ...
    public void registerCitizen(Cittadino user){
        try
        {
            try
            {
                ResultSet rs=conn.prepareStatement("SELECT idCittadino FROM Cittadini_Registrati WHERE userid='"+user.getUserid()+"'").executeQuery();
                rs.next();
            ...
            {
                Statement statement = conn.createStatement();
                ResultSet rs=conn.prepareStatement("SELECT idCittadino FROM Cittadini_Registrati ORDER BY idCittadino DESC").executeQuery();
                int id;
                try
                {
                    rs.next();
    ...
    public void registerCenter(CentroVaccinale center)
    {
        try
        {
            ...
            try
            {
                ResultSet rs=conn.prepareStatement("SELECT idCentroVaccinale FROM CentriVaccinali WHERE nome='"+center.getNomeCentro()+"'").executeQuery();
                rs.next();
            ...
            if(check)
            {
                try
                {
                    ResultSet rs=conn.prepareStatement("SELECT idCentroVaccinale FROM CentriVaccinali cv JOIN (Indirizzi i JOIN Comuni com ON i.idComune=com.idComune) ON cv.idIndirizzo=i.idIndirizzo WHERE cv.nome='"+center.getNomeCentro()+"' AND  com.nome='"+center.getComune()+"' AND com.cap='"+center.getCap()+"' AND i.nome='"+center.getNome()+"' AND i.qualificatore='"+center.getQualificatore()+"' AND i.n_civico='"+center.getnCivico()+"'").executeQuery();
                    rs.next();
                    rs.getInt("idCentroVaccinale");
                ...
                {
                    ResultSet rs=conn.prepareStatement("SELECT idCentroVaccinale FROM CentriVaccinali ORDER BY idCentroVaccinale DESC").executeQuery();
                    int id;
                    try
                    {
                        rs.next();
                    ...
                    try
                    {
                        rs=conn.prepareStatement("SELECT idIndirizzo FROM Indirizzi i JOIN Comuni com ON i.idComune=com.idComune WHERE i.nome='"+center.getNome()+"' AND i.qualificatore='"+center.getQualificatore()+"' AND i.n_civico='"+center.getnCivico()+"' AND com.nome='"+center.getComune()+"' AND com.cap='"+center.getCap()+"'").executeQuery();
                        rs.next();
                    ...
                    catch(Exception ex1)
                    {
                        try
                        {
                            rs=conn.prepareStatement("SELECT idIndirizzo FROM Indirizzi ORDER BY idIndirizzo DESC").executeQuery();
                            rs.next();
                            indirizzo=rs.getInt("idIndirizzo");
                        }
                        catch(Exception ex2)
                        ...
                        try
                        {
                            rs=conn.prepareStatement("SELECT idComune FROM Comuni WHERE nome='"+center.getComune()+"' AND cap='"+center.getCap()+"'").executeQuery();
                            rs.next();
                        ...
                        catch(Exception ex2)
                        {
                            try
                            {
                                rs=conn.prepareStatement("SELECT idComune FROM Comuni ORDER BY idComune DESC").executeQuery();
                                rs.next();
                                comune=rs.getInt("idComune");
                            }
                            catch(Exception ex3)
    ...
    public void registerVaccination(Cittadino user, CentroVaccinale center)
    {
        try
        {
            ...
            {
                ResultSet rs=conn.prepareStatement("SELECT idVaccinazione FROM "+table+" ORDER BY idVaccinazione DESC").executeQuery();
                try
                {
                    rs.next();
            ...
            try
            {
                ResultSet rs=conn.prepareStatement("SELECT idVaccinazione FROM "+table+" v JOIN Cittadini_Registrati cr ON v.idCittadino=cr.idCittadino WHERE cr.userid='"+user.getUserid()+"'").executeQuery();
                rs.next();
            ...
            catch(Exception ex)
            {
                ...
                try
                {
                    ...
                    rs=conn.prepareStatement("SELECT idCittadino FROM Cittadini_Registrati WHERE userid='"+user.getUserid()+"'").executeQuery();
                    rs.next();
                    cittadino=rs.getInt("idCittadino");
                    statement.executeUpdate
                    (
                        "INSERT INTO "+table+" (idVaccinazione, idCentroVaccinale, idCittadino, idTipologia, data_somministrazione) VALUES ("+
                        id+", "+
                        centrovaccinale+", "+
                        cittadino+", "+
                        user.getTipo().ordinal()+", '"+
                        user.getDataSomministrazione().split("/")[2]+"-"+user.getDataSomministrazione().split("/")[1]+"-"+user.getDataSomministrazione().split("/")[0]+"')"
                    );
    ...
    public void insertAdverseEvent(Cittadino citizen, CentroVaccinale center, EventoAvverso event)
    {
        try
        {
            try
            {
                ResultSet rs=conn.prepareStatement("SELECT idEvento FROM EventiAvversi e JOIN Cittadini_Registrati cr ON e.idCittadino=cr.idCittadino WHERE cr.userid='"+citizen.getUserid()+"' AND e.idTipologia='"+event.getEvento().ordinal()+"'").executeQuery();
                rs.next();
                rs.getInt("idEvento");
            ...
            {
                int id;
                ResultSet rs=conn.prepareStatement("SELECT idEvento FROM EventiAvversi ORDER BY idEvento DESC").executeQuery();
                try
                {
                    rs.next();
                ...
                try
                {
                    rs=conn.prepareStatement("SELECT idCentroVaccinale FROM CentriVaccinali WHERE nome='"+center.getNomeCentro()+"'").executeQuery();
                    rs.next();
                    centrovaccinale=rs.getInt("idCentroVaccinale");
                    rs=conn.prepareStatement("SELECT idCittadino FROM Cittadini_Registrati WHERE userid='"+citizen.getUserid()+"'").executeQuery();
                    rs.next();
                    cittadino=rs.getInt("idCittadino");
                    statement.executeUpdate
                    (
    ...
    public ArrayList<CentroVaccinale> getCentersByName(String name) 
    {
        ...
        try
        {
            ...
            (
                ...
                "com.provincia AS comprovincia, "+
                "com.cap AS comcap "+
                "FROM CentriVaccinali cv JOIN (Indirizzi i JOIN Comuni com ON i.idComune=com.idComune) ON cv.idIndirizzo=i.idIndirizzo WHERE cv.nome LIKE '%"+name+"%'"
            ).executeQuery();
            while(rs.next())
            {
                TipoCentroVaccinale tipo=null;
    ...
    public ArrayList<Cittadino> getCitizens()
    {
        ...
        try
        {
            ResultSet rs=conn.prepareStatement
            (
                "SELECT * FROM Cittadini_Registrati"
            ).executeQuery();
            while(rs.next())
            {
                output.add
    ...
    public ArrayList<EventoAvverso> getAdverseEvents(CentroVaccinale center)
    {
        ...
        try
        {
            ResultSet rs=conn.prepareStatement
            (
                "SELECT ev.* FROM EventiAvversi ev JOIN CentriVaccinali cv ON ev.idCentroVaccinale=cv.idCentroVaccinale WHERE cv.nome LIKE '%"+center.getNomeCentro()+"%'"
            ).executeQuery();
            while(rs.next())
            {
        ...
        try
        {
            ...
            while(rs.next())
            {
                ...
                (
                    new Cittadino
                    (
                        rs.getString("nome"), rs.getString("cognome"), "", rs.getString("email"), rs.getString("userid"), rs.getString("password"), rs.getLong("idVaccinazione"), rs.getString("data_somministrazione").split("-")[2]+"/"+rs.getString("data_somministrazione").split("-")[1]+"/"+rs.getString("data_somministrazione").split("-")[0], tipo
                    )
                );
            }
        }
